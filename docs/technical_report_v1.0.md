...
6. **Quarterly review checklist created** — `docs/reference/QUARTERLY_REVIEW.md` provides a structured process for maintaining all controlled vocabularies, ordered from lowest-risk (company watchlist, no code changes) to highest-risk (region enums, run tests after).
...
| Companies, OEMs, tech partners | `docs/reference/company-watchlist.md` | Human analysts (quarterly review) |
...
4. **Company watchlist** → kept as the sole remaining Markdown reference file (`docs/reference/company-watchlist.md`), stripped of all content that duplicated code (outdated regions, priority rules).
...
**Workaround for the transition:** Rather than deleting everything immediately, the outdated spec files were moved to `docs/reference/archive/` to preserve project history. A `QUARTERLY_REVIEW.md` checklist was created to formalize the ongoing maintenance process, ordered from lowest-risk changes (company watchlist — pure reference, no code) to highest-risk (region enums — run tests after every change).
