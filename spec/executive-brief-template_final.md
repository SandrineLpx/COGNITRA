# Executive Brief Template (Closure Systems Focus)

Standard format for:
1) Individual Intelligence Briefs (one per document)
2) Weekly Executive Reports (digest)

No emojis. Keep the language executive-friendly and evidence-based.

---

## A) Intelligence Brief (single document)

```markdown
INTELLIGENCE BRIEF
Summary Date: [YYYY-MM-DD]
Source Type: [Bloomberg | Automotive News | Reuters | Patent | Press Release | Other]
Publish Date: [YYYY-MM-DD or Unknown]
Original URL: [optional]

Title
[Document title]

Companies Mentioned
- [Company 1]
- [Company 2]

Actor Type
- [oem | supplier | tech_partner | government | regulator | industry_group | media | other]
Government/Regulators (if applicable)
- [Entity]

Geography
Countries Mentioned: [list]
Regions Relevant (Footprint): [India | China | Europe (including Russia) | Africa | US | Mexico | Thailand]
Region Signal Type: [trade_policy | geopolitics | regulation | oem_activity | supplier_activity | demand | logistics | fx_financing]
Supply Flow Hint (optional): [e.g., China->EU]

Topics Covered (canonical)
- [1–3 topics from the canonical list]

Priority and Confidence
Priority: [High | Medium | Low]
Confidence: [High | Medium | Low]

Evidence (verifiable facts)
- [2–4 bullets; include short quote fragments if available]

Key Insights (interpretation)
- [2–4 bullets]

Strategic Implications (closure systems supplier lens)
- [2–4 bullets]

Recommended Actions (optional but preferred)
- [1–3 bullets]

Review
Reviewed By: [name]
Review Status: [Not Reviewed | Reviewed | Approved]
Notes (optional): [free text]
```

---

## B) Weekly Executive Report (digest)

```markdown
AUTOMOTIVE COMPETITIVE INTELLIGENCE BRIEF
Period: [Start Date] to [End Date]
Prepared by: [Name/Role]

============================================================

EXECUTIVE SUMMARY
[2–3 sentences highlighting the most significant developments and why they matter.]

HIGH PRIORITY DEVELOPMENTS
- [3–6 bullets max; each bullet should mention company/OEM + what happened + why it matters.]

FOOTPRINT REGION SIGNALS
Europe (including Russia)
- [1–3 bullets; include Countries: ... when applicable]

US
- [1–3 bullets]

Mexico
- [1–3 bullets]

China
- [1–3 bullets]

India
- [1–3 bullets]

Thailand
- [1–3 bullets]

Africa
- [1–3 bullets; primarily Morocco and South Africa]

KEY DEVELOPMENTS BY TOPIC
Include only topics present this period. Use canonical labels exactly.

OEM Strategy & Powertrain Shifts
- [Key developments]

Closure Technology & Innovation
- [Key developments]

OEM Programs & Vehicle Platforms
- [Key developments]

Regulatory & Safety
- [Key developments]

Supply Chain & Manufacturing
- [Key developments]

Technology Partnerships & Components
- [Key developments]

Market & Competition
- [Key developments]

Financial & Business Performance
- [Key developments]

Executive & Organizational
- [Key developments]

EMERGING TRENDS
- [1–3 bullets. Trends should be based on multiple items or clear signal strength.]

RECOMMENDED ACTIONS
- [3–6 bullets; who should do what next.]

APPENDIX (optional)
Items Covered: [#]
Method: Structured extraction from source documents; human review and approval.
```
